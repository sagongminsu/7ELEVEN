...
public class NPC : MonoBehaviour // ,IDamagable 추가해야 함 - 에러나서 주석처리해둠
{
    [Header("Stats")]
    public int health;
    ...
    //        if (agent.CalculatePath(PlayerController.instace.transform.position, path)) // 경로 체크해보고 경로가 가능한 경로면 그 경로로 이동하고 아니면 도망간다
    //        {
    ...
    //            PlayerController.instance.GetComponent<IDamagable>().TakePhysicalDamage(damage); // 데미지 전달
    //            animator.speed = 1;
